...
type Challenge = { from: string; to: string; prompt: string; index: number; status: 'pending' | 'accepted' };
...
app.get('/api/health', c => c.json({ ok: true, version: 'v0.5' }));
  ...
  async webSocketMessage(ws: WebSocket, raw: string | ArrayBuffer) {
    ...
    if (m.type === 'join') {
      ...
      if (m.type === 'ready' && s.phase === 'lobby') me.ready = !me.ready;
        ...
        s.challenge = { from: me.id, to: m.target, prompt: challenges[index], index, status: 'pending' };

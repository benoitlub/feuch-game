...
type Saved = { players: Player[]; hostId: string | null; phase: Phase; round: number; nightTarget: string | null; linked: string[]; natashaUsed: boolean; dayVotes: Record<string,string>; winner: string | null };
const initial = (): Saved => ({ players: [], hostId: null, phase: 'lobby', round: 0, nightTarget: null, linked: [], natashaUsed: false, dayVotes: {}, winner: null });
const app = new Hono<{ Bindings: Env }>();
app.get('/api/health', c => c.json({ ok: true, version: 'v0.2' }));
...
export class GameRoom extends DurableObject {
  ...
  private broadcast() {
    ...
    for (const [ws, id] of this.sockets) {
      ...
      this.send(ws, { type: 'state', phase: s.phase, round: s.round, players, hostId: s.hostId, me: me ? { id: me.id, role: me.role, camp: me.camp } : null, linked: me && s.linked.includes(me.id) ? s.linked : [], natashaUsed: me?.role === 'Natasha' ? s.natashaUsed : undefined, voted: Boolean(s.dayVotes[id]), winner: s.winner });
    }
  }
    ...
    if (m.type === 'join') {
      ...
      if (m.type === 'ready' && s.phase === 'lobby') me.ready = !me.ready;
        ...
        if (s.phase === 'night') {
          ...
          const counts = Object.values(s.dayVotes).reduce<Record<string,number>>((a,id) => { a[id] = (a[id] ?? 0) + 1; return a; }, {});
          const max = Math.max(0, ...Object.values(counts));
          const leaders = Object.keys(counts).filter(id => counts[id] === max);
          if (max && leaders.length === 1) {
            const target = s.players.find(p => p.id === leaders[0]);
            if (target && !s.linked.includes(target.id)) { target.camp = 'feuch'; this.send(ws, { type: 'notice', message: target.name + ' rejoint le camp Feuch (règle V0.2 provisoire).' }); }
          }
          s.dayVotes = {}; s.phase = 'night'; s.round++;
        }

...
type Challenge = { from: string; to: string; prompt: string; index: number };
...
type Saved = { players: Player[]; hostId: string | null; phase: Phase; round: number; nightTarget: string | null; linked: string[]; natashaUsed: boolean; dayVotes: Record<string,string>; conversionUsed: boolean; challenge: Challenge | null; challengeUsed: string[]; vice: Record<string,number>; winner: string | null };
const initial = (): Saved => ({ players: [], hostId: null, phase: 'lobby', round: 0, nightTarget: null, linked: [], natashaUsed: false, dayVotes: {}, conversionUsed: false, challenge: null, challengeUsed: [], vice: {}, winner: null });
const app = new Hono<{ Bindings: Env }>();
app.get('/api/health', c => c.json({ ok: true, version: 'v0.4' }));
  ...
  async webSocketMessage(ws: WebSocket, raw: string | ArrayBuffer) {
    ...
    if (m.type === 'join') {
      ...
      if (m.type === 'ready' && s.phase === 'lobby') me.ready = !me.ready;
        ...
        s.phase = 'night'; s.round = 1; s.conversionUsed = false; s.challenge = null; s.challengeUsed = []; s.vice = {};
        ...
        s.challenge = { from: me.id, to: m.target, prompt: challenges[index], index };
        ...
        const winner = m.other === 'accept' ? me.id : s.challenge.from;
        s.vice[winner] = (s.vice[winner] ?? 0) + 1;
        const result = m.other === 'accept' ? 'Défi accepté : 1 Vice au joueur défié (règle de test).' : 'Défi refusé : 1 Vice au provocateur.';
        s.challenge = null;
